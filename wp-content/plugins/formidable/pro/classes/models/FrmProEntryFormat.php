...
class FrmProEntryFormat {
	...
	public static function prepare_entry_array( $values, $atts ) {
		$field = $atts['field'];
		$in_section = ! FrmField::is_option_empty_in_object( $field, 'in_section' );
		$in_embedded_form = $field->id != $atts['form_id'];

		if ( isset( $atts['entry']->sub_entries ) && ( $in_section || $in_embedded_form ) ) {
			...
			foreach ( $atts['entry']->sub_entries as $sub_id => $sub_entry ) {
				if ( ! $atts['include_blank'] && $sub_entry[ $field->id ] == '' ) {
					continue;
				}
